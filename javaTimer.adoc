...
Here I want to study the `java.util.concurrent.Timer` class (henceforth just `Timer`) and distill it to fundamentals. I want to answer the questions:
...
** Correspondingly, the [.underline]#native#  method `notifyAll()` is the mechanism by which the thread can be "waken up, if during its sleep," a new task is submitted that ought to run sooner.

== The MVP of a `Timer`

We can begin by thinking what is the purpose of `Timer`? Well, it is to receive and manage tasks with deferred and/or periodic execution. Given an instance, you want to give it a task "to run 1 minute from now", or "every 30 seconds."
...
The diagram below is appropriate representation:
...
NOTE: Since Java 5.0, `Timer` went out of favor with the introduction of `java.util.concurrent.ScheduledThreadPoolExecutor`. The latter is more versatile, but unless you need its extra-capabilities, `Timer` is more than adequate. Another advantage of `ScheduledThreadPoolExecutor` is that it is an interface whilst `Timer` is a concrete class. Testing your own class is much easier when you depend on an interface and inject a dummy implementation of `ScheduledThreadPoolExecutor`. With `Timer`, that is much harder.
...
* `schedule(<task>, <delay>, <period>)`, and
* `scheduleAtFixedRate(<task>, <delay>, <period>)`.

For the first, the `period` represents the time that should elapse between the execution of each task.
For the second, the `period` represents the frequency (e.g. rate) at which the task should run.
